...
//Public Methods
	...
	public int getTraitAptitudeValue(String gTraitAptitudeName)
	{
		//If the Trait already has a Aptitude with the same name as the Aptitude being received
		if(getTraitAptitudesStringAptitudeHashMap().containsKey(gTraitAptitudeName))
		{
			//Return the Value of the APtitude for this Trait
			return getTraitAptitudesStringAptitudeHashMap().get(gTraitAptitudeName).getAptitudeValue();
		}
		//The Aptitude does not exist in this Trait
		else
		{
			//Return a value of zero since the Aptitude does not exist
			return 0;
		}
	}
